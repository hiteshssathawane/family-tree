...
| T-32 | **`--headless` runs submit Gender / Status / Marital Status blank.** The click itself was fixed under T-03 and `clickRadio` now asserts `aria-checked` flipped (`fill-form.js:272,286-290`), yet the values still arrive empty in the Sheet on headless runs. Headed runs (`slowMo: 200`, `fill-form.js:409`) have never shown it, so headless timing is the suspected variable — **untested, not yet reproduced under instrumentation**. Next step: run one headless row with a trace and check whether `aria-checked` is true at submit time but the radio's hidden input is never populated. Not on any critical path — the 64-row load is already in, and `npm run form:next` headed is the working path | Medium | `scripts/fill-form.js` | TODO |
